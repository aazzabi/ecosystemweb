...
class DACommandeController extends Controller
{
    ...
    public function showLigneAction($id_c)
    {
        ...
        return $this->render('@Eco/DashboardAdmin/Commande/da_detail_commande.html.twig',array(
    ...
    public function statAction()
    {
        ...
        $em = $this->getDoctrine()->getManager();
        //nb1
        $RAW_QUERY = 'SELECT  COUNT(*) as nb1 from commande where (date_emission >= \'2019-01-01\') AND (date_emission <= \'2019-01-31\');';
        $statement = $em->getConnection()->prepare($RAW_QUERY);
        $statement->execute();
        $nb1 = $statement->fetch();
        //nb2
        $RAW_QUERY2 = 'SELECT  COUNT(*) as nb2  from commande where (date_emission >= \'2019-02-01\') AND (date_emission <= \'2019-02-31\');';
        ...
        //nb5
        $RAW_QUERY5 = 'SELECT  COUNT(*) as nb5  from commande where (date_emission >= \'2019-05-01\') AND (date_emission <= \'2019-05-31\');';
        $statement5 = $em->getConnection()->prepare($RAW_QUERY5);
        $statement5->execute();
        $nb5 = $statement5->fetch();

        //nb6
        $RAW_QUERY6 = 'SELECT  COUNT(*) as nb6  from commande where (date_emission >= \'2019-06-01\') AND (date_emission <= \'2019-06-31\');';
        $statement6 = $em->getConnection()->prepare($RAW_QUERY6);
        $statement6->execute();
        $nb6 = $statement6->fetch();

        //nb7
        $RAW_QUERY7 = 'SELECT  COUNT(*) as nb7  from commande where (date_emission >= \'2019-07-01\') AND (date_emission <= \'2019-07-31\');';
        $statement7= $em->getConnection()->prepare($RAW_QUERY7);
        $statement7->execute();
        $nb7 = $statement7->fetch();

        //nb8
        $RAW_QUERY8 = 'SELECT  COUNT(*) as nb8  from commande where (date_emission >= \'2019-08-01\') AND (date_emission <= \'2019-08-31\');';
        $statement8= $em->getConnection()->prepare($RAW_QUERY8);
        $statement8->execute();
        $nb8 = $statement8->fetch();

        //nb9
        $RAW_QUERY9 = 'SELECT  COUNT(*) as nb9  from commande where (date_emission >= \'2019-09-01\') AND (date_emission <= \'2019-09-31\');';
        $statement9= $em->getConnection()->prepare($RAW_QUERY9);
        $statement9->execute();
        $nb9 = $statement8->fetch();

        //nb10
        $RAW_QUERY10 = 'SELECT  COUNT(*) as nb10  from commande where (date_emission >= \'2019-10-01\') AND (date_emission <= \'2019-10-31\');';
        $statement10= $em->getConnection()->prepare($RAW_QUERY10);
        $statement10->execute();
        $nb10 = $statement10->fetch();

        //nb11
        $RAW_QUERY11 = 'SELECT  COUNT(*) as nb11  from commande where (date_emission >= \'2019-11-01\') AND (date_emission <= \'2019-11-31\');';
        $statement11= $em->getConnection()->prepare($RAW_QUERY11);
        $statement11->execute();
        $nb11 = $statement11->fetch();

        //nb12
        $RAW_QUERY12 = 'SELECT  COUNT(*) as nb12  from commande where (date_emission >= \'2019-12-01\') AND (date_emission <= \'2019-12-31\');';
        ...
        return $this->render('@Eco/DashboardAdmin/Commande/stat_commande.html.twig',array('tab'=> $tab));
